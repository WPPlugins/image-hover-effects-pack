jQuery(document).ready(function($) {
    ...
    $('.respimages').each(function(index, el) {
        if ($(this).val()==='yes') {
            $(this).closest('.form-table').find('.customwidth,.customheight').hide();
        }else if($(this).val()==='no'){
            $(this).closest('.form-table').find('.customwidth,.customheight').show();           
        };
    });
    ...
    $('#caption').on('change', '.respimages', function(event) {
        event.preventDefault();
        if ($(this).val()=='yes') {
            $(this).closest('.form-table').find('.customwidth,.customheight').hide();
        }else if($(this).val()=='no'){
    ...
    jQuery('#caption').on('click','.bgimage',function( event ){
     
        event.preventDefault();
     
         var parent = jQuery(this).closest('.ui-accordion-content').find('.backgroundimage');
            // Create the media frame.
            la_caption_hover = wp.media.frames.la_caption_hover = wp.media({
              title: 'Select Image for Caption Background',
              button: {
            ...
            la_caption_hover.on( 'select', function() {
                ...
                selection.map( function( attachment ) {
                    ...
                    parent.append('<span><img style="max-width: 135px;max-height: 135px;" src="'+attachment.url+'"><span class="dashicons dashicons-dismiss" title="Remove Image"></span></span>');
    ...
    jQuery('.head-color,.desc-color,.capbordercolor,.headingbg').wpColorPicker(); 
        ...
        var allcats = []; 
          jQuery('.accordian>.content').each(function(index,val) {
            ...
            jQuery(this).find('.ui-accordion-content').each(function(index, val) {
                ...
                images.cap_head_animation = jQuery(this).find('.headinganim').val();
                images.cap_desc = jQuery(this).find('.capdesc').val();
                images.cap_desc_animation = jQuery(this).find('.capanim').val();
                images.cap_link = jQuery(this).find('.caplink').val();
                images.cap_newtab = jQuery(this).find('.capnewtab').val();
                images.cap_headsize = jQuery(this).find('.headfontsize').val();
                images.cap_descsize = jQuery(this).find('.descfontsize').val();
                images.cap_resp_option = jQuery(this).find('.respimages').val();
                images.cap_thumbwidth = jQuery(this).find('.thumbwidth').val();
                images.cap_thumbheight = jQuery(this).find('.thumbheight').val();
                images.cap_effect = jQuery(this).find('.effectopt').val();
                images.cap_border_width_square = jQuery(this).find('.capborderwidthsq').val();
                images.cap_border_color = jQuery(this).find('.capbordercolor').val();
                ...
                images.cap_bgcolor = jQuery(this).find('.capbgcolor').val();
                images.shortcode = jQuery(this).find('.fullshortcode').attr('id');
                images.capbgtype = jQuery(this).find('.capbgtype').val();
                images.cap_bg_img = jQuery(this).find('.backgroundimage').find('img').attr('src');
                cats.allcapImages.push(images); 
            });
            allcats.push(cats);
        });
        var data = {
            action : 'la_save_hover_effects_pack',
             posts : allcats       
        } 
    ...
    jQuery('.content').on('click','button.fullshortcode',function(event) {
        event.preventDefault();
        prompt("Copy and use this Shortcode", '[hover-effects-pack id="'+jQuery(this).attr('id')+'"]');
    });
